...
def scheduler_wrapper(pipeline, schedulers, name="_scheduler_wrapper", gamma=2.0):
    ...
    def step(self, model_output, timestep, sample, **kwargs):
        ...
        if self.is_last_timestep(timestep) and not self.is_last_stage():
            ...
            osh, osw = schedulers[self._msp_i - 1]["size"]
            tsh, tsw = schedulers[self._msp_i]["size"]
            ...
            # Resize to the next stage's (H, W). Using only height as a uniform
            # scale_factor breaks non-square targets (e.g. 1024² -> 2048x4096).
            images = torch.nn.functional.interpolate(
                images, size=(tsh, tsw), mode="nearest"
            )
